...
public class UpdateDocumentConsumerTest extends CamelTestSupport {
    ...
    private UpdateDocumentRequest request = new UpdateDocumentRequest(documentUUID, DocumentStatus.UPLOADED,
        "oldfile.docx", "some.pdf");
    ...
    public void shouldCallDocumentDataService() throws Exception {
        doNothing().when(documentDataService).updateDocument(documentUUID, DocumentStatus.UPLOADED, "oldfile.docx",
            "some.pdf");

        template.sendBody(endpoint, request);
        verify(documentDataService, times(1)).updateDocument(documentUUID, DocumentStatus.UPLOADED, "oldfile.docx",
            "some.pdf");
    }
